...
def zins():
    ...
    e = int(input("Möchten Sie wissen wann ihr Gehalt einen bestimmten Wert erreicht (0) oder wie hoch Ihr Vermögen "
                  "nach einer bestimmten Zeit ist (1)?\n"))
    if e == 1:
...
def prim_zahl_tester():
    ...
    for i in range(2, (zahl - 1)):
        if zahl % i == 0:
            is_prim = False
            break
    print(f'Deine Zahl ({zahl}) ist {"eine" if is_prim else "keine"} Primzahl')